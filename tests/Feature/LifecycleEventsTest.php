...
beforeEach(function (): void {
    $this->dir = sys_get_temp_dir() . '/installer-events-' . uniqid();
...
it('dispatches started, user-created and finished across a full run', function (): void {
    Event::fake([InstallerStarted::class, UserCreated::class, InstallerFinished::class]);

    app(InstallerEngine::class)->run(($this->context)());

    Event::assertDispatched(InstallerStarted::class, 1);
    Event::assertDispatched(InstallerFinished::class, 1);
    Event::assertDispatched(
        UserCreated::class,
        fn (UserCreated $e): bool => $e->data->email === 'ada@example.com' && is_object($e->user),
    );
});

it('dispatches InstallerFailed (and StepFailed) when a step throws', function (): void {
    config()->set('installer.user.model', stdClass::class); // not an Eloquent model → user step throws

    Event::fake([InstallerFailed::class, StepFailed::class]);
    ...
    Event::assertDispatched(InstallerFailed::class, fn (InstallerFailed $e): bool => $e->step !== null);
});
